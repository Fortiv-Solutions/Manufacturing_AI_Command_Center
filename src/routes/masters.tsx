...
export const Route = createFileRoute("/masters")({
  ...
  component: () => (
    <div className="max-w-[1480px] mx-auto">
      <PageHeader breadcrumbs={[{ label: "Home", to: "/" }, { label: "Masters" }]} title="Master data" subtitle="Items, BOMs, vendors, customers, dealers and plant structure." />
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {cards.map((c) => (
          <Link key={c.title} to="/" className="rounded-2xl border bg-card p-5 hover:border-primary/40 transition">
