...
import {
  BookOpen, Search, Sparkles, Loader2, Tag,
  User, Clock, ThumbsUp, MessageSquare, Plus, Star, ChevronRight
} from "lucide-react";
...
const ARTICLES: Article[] = [
  {
    ...
    summary: "Following the Well C-07 ESP workover (Oct 2025), ramp-up from 0 to target rate over 72 hrs using 5% increments every 8 hrs prevented motor burnout. This supersedes the legacy 24-hr protocol.",
    confidence: 97,
    votes: 14,
    comments: 3,
    validated: true,
  ...
  },
  {
    ...
    summary: "Injection shortfall on Pad B (Oct 2025) systematically diagnosed to impeller wear on injection pump P-B02. Decision tree and inspection checklist developed for future recurrences.",
    confidence: 91,
    votes: 9,
    comments: 5,
    validated: true,
  ...
  },
  {
    ...
    summary: "Documented procedure for filing the DGH W-10 monthly production return for OML-11, including SAP data pull steps, typical validation errors, and submission portal walkthrough.",
    confidence: 99,
    votes: 21,
    comments: 8,
    validated: true
  },
  {
    ...
    summary: "Standard mobilisation sequence from Jodhpur yard for B-cluster well spuds. Lead time benchmarked at 72 hours from approval. Route clearances, escort requirements, and permit checklist included.",
    confidence: 89,
    votes: 6,
    comments: 2,
    validated: false,
  ...
  },
  {
    ...
    summary: "Monthly audit checklist for Pad C chemical storage yards including MSDS verification, container integrity inspection, and tolerance limits for corrosion inhibitor stock levels.",
    confidence: 93,
    votes: 11,
    comments: 4,
    validated: true
...
const CATEGORIES = ["All", "Well Engineering", "Production Engineering", "Drilling", "Regulatory", "HSE"];
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div className="lg:col-span-4 space-y-4">
        ...
        <div className="rounded-2xl border border-[#D1D9E3] bg-white p-1.5 flex shadow-sm">
          ...
          >
            <BookOpen className="h-3.5 w-3.5 inline mr-1.5" />Browse
          </button>
          <button
          ...
          >
            <Plus className="h-3.5 w-3.5 inline mr-1.5" />Contribute
          </button>
        </div>
        ...
        {activeSection === "browse" ? (
          <>
            ...
            <div className="space-y-2">
              {filtered.length === 0 ? (
                <div className="py-8 text-center text-xs text-[#8892A0]">No articles match your search.</div>
              ) : filtered.map(a => (
                <button
                  key={a.id}
                  ...
                  <div className="flex items-start gap-2">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 mb-1">
                        <Badge className={`${getCategoryBadge(a.category)} text-[9px] font-bold border`}>{a.category}</Badge>
                        {a.validated && <Star className="h-3 w-3 text-[#C8922A] fill-[#C8922A]" />}
                      </div>
                      <p className="text-xs font-bold text-[#0D1B2A] line-clamp-2 leading-snug">{a.title}</p>
                      <div className="flex items-center gap-2 mt-1.5 text-[10px] text-[#8892A0]">
                        <span className="flex items-center gap-0.5"><User className="h-3 w-3" />{a.author}</span>
                        <span className="flex items-center gap-0.5"><Clock className="h-3 w-3" />{a.date}</span>
                      </div>
                    </div>
          ...
          <div className="rounded-2xl border border-[#D1D9E3] bg-white p-5 shadow-sm space-y-4">
            <div>
              ...
              <p className="text-[10.5px] text-[#8892A0] leading-relaxed">Describe an issue you solved, a technique that worked, or a regulatory insight. AI will structure it into a knowledge article.</p>
            </div>

            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-[#1A6B8A] uppercase tracking-wider">Title (optional)</label>
              <input
                type="text"
                value={newTitle}
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-[#1A6B8A] uppercase tracking-wider">Experience / Observation</label>
              <textarea
                value={newContent}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-[#1A6B8A] uppercase tracking-wider">Category</label>
                <select className="w-full h-9 rounded-lg border border-[#D1D9E3] bg-white px-2.5 text-xs">
                  {CATEGORIES.filter(c => c !== "All").map(c => <option key={c}>{c}</option>)}
                </select>
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-[#1A6B8A] uppercase tracking-wider">Well / Asset Ref</label>
                <input
                  type="text"
            ...
            >
              {isGenerating ? (
                <><Loader2 className="h-4 w-4 animate-spin mr-1.5" />Structuring Article...</>
              ) : (
                <><Sparkles className="h-4 w-4 mr-1.5" />Capture & Structure with AI</>
              )}
            </Button>
          </div>
        )}
        ...
        {!selectedArticle ? (
          ...
          <>
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-2 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <Badge className={`${getCategoryBadge(selectedArticle.category)} border text-[9.5px] font-bold`}>{selectedArticle.category}</Badge>
                    ...
                    <span className="text-[10px] font-mono text-[#8892A0]">{selectedArticle.id}</span>
                  </div>
                  <h3 className="text-sm font-bold text-[#0D1B2A] leading-snug">{selectedArticle.title}</h3>
                  <div className="flex items-center gap-3 text-[10.5px] text-[#8892A0]">
                    <span className="flex items-center gap-1"><User className="h-3 w-3" /> <strong className="text-[#1A6B8A]">{selectedArticle.author}</strong></span>
                    <span className="flex items-center gap-1"><Clock className="h-3 w-3" />{selectedArticle.date}</span>
                    {selectedArticle.wellRef && <span className="flex items-center gap-1"><Tag className="h-3 w-3" />{selectedArticle.wellRef}</span>}
                  </div>
                </div>

                {/* AI Confidence */}
                <div className="shrink-0 text-center">
                  <p className="text-[9.5px] font-bold text-[#8892A0] uppercase mb-1">AI Confidence</p>
                  <div className="relative h-16 w-16 mx-auto">
                    <svg viewBox="0 0 36 36" className="h-16 w-16 -rotate-90">
                      <circle cx="18" cy="18" r="15.9155" fill="none" stroke="#F0F2F5" strokeWidth="3" />
                      <circle
                        cx="18" cy="18" r="15.9155" fill="none"
                        stroke={selectedArticle.confidence >= 95 ? "#0F7B6C" : selectedArticle.confidence >= 85 ? "#C8922A" : "#C0392B"}
                        strokeWidth="3"
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <span className="text-xs font-bold font-mono text-[#0D1B2A]">{selectedArticle.confidence}%</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="flex-1 p-5 overflow-y-auto space-y-5">
              ...
              {/* Summary */}
              <div>
                <h4 className="text-[10px] font-bold text-[#1A6B8A] uppercase tracking-wider mb-2">Summary</h4>
                <p className="text-xs text-[#0D1B2A] leading-relaxed border-l-4 border-[#1A6B8A] pl-3 bg-blue-50/30 py-2 rounded-r-lg">{selectedArticle.summary}</p>
              </div>

              {/* Tags */}
              <div>
                <h4 className="text-[10px] font-bold text-[#1A6B8A] uppercase tracking-wider mb-2">Entity Tags (AI-Extracted)</h4>
                <div className="flex flex-wrap gap-1.5">
                  {selectedArticle.tags.map(tag => (
                    <span key={tag} className="flex items-center gap-1 px-2.5 py-1 rounded-full bg-slate-100 border border-[#D1D9E3] text-[10.5px] font-semibold text-[#0D1B2A]">
                      <Tag className="h-2.5 w-2.5 text-[#C8922A]" />{tag}
                    </span>
                  ))}
                </div>
              </div>
              ...
              <div>
                <h4 className="text-[10px] font-bold text-[#1A6B8A] uppercase tracking-wider mb-2">Related Articles</h4>
                <div className="space-y-1.5">
                  {ARTICLES.filter(a => a.id !== selectedArticle.id).slice(0, 2).map(a => (
                    <button key={a.id} onClick={() => setSelectedArticle(a)} className="w-full text-left flex items-center gap-2 p-2.5 rounded-lg border border-[#D1D9E3] hover:bg-slate-50 transition-colors">
            ...
            <div className="p-4 border-t border-[#D1D9E3] bg-white flex items-center justify-between gap-3">
              <div className="flex items-center gap-3 text-[10.5px] text-[#8892A0]">
                <button onClick={() => handleVote(selectedArticle)} className="flex items-center gap-1.5 hover:text-[#0F7B6C] transition-colors font-semibold">
              ...
              <Button size="sm" className="h-8 bg-[#1A6B8A] hover:bg-[#155a75] text-white text-xs font-bold"
