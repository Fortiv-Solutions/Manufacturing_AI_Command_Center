...
import { 
  AlertTriangle, ShieldCheck, Database, FileText, ChevronRight, 
  HelpCircle, User, Calendar, Check, Download, Grid, ArrowLeft, RefreshCw
} from "lucide-react";
  ...
  const [anomalies, setAnomalies] = useState<AnomalyItem[]>([
    {
      ...
      severity: "High",
      status: "Pending",
      extractedText: '"The Contractor shall be entitled to recover from Cost Petroleum an amount not exceeding ninety percent (90%) of the total Petroleum produced and saved in any Calendar Year, in respect of costs incurred in accordance with the Accounting Procedure set out in Exhibit C."',
      standardText: '"The Contractor shall be entitled to recover one hundred percent (100%) of total Petroleum produced and saved in any Calendar Year in respect of allowable costs..."',
      impactAnalysis: "The cost recovery cap of 90% is 10 percentage points below the standard NELP template. At projected Phase 3 production of approximately 12,000–18,000 bbl/day, this deviation results in a material unrecoverable cost exposure estimated at Rs. 4.2–6.8 Cr annually at current prices.",
    ...
    },
    {
      ...
      severity: "High",
      status: "In Review",
      extractedText: '"Any dispute arising out of this contract shall be referred to arbitration under UNCITRAL rules, with the seat of arbitration being Singapore, and proceedings conducted in English."',
      standardText: '"All disputes shall be referred to arbitration in accordance with the Indian Arbitration and Conciliation Act 1996, with the seat of arbitration being New Delhi, India."',
      impactAnalysis: "Foreign arbitration seat creates jurisdictional complexity. All disputes governed by Singapore Arbitration Act; higher litigation costs; Indian courts have limited supervisory jurisdiction.",
      portfolioDistribution: [
    ...
    },
    {
      ...
      severity: "Medium",
      status: "Pending",
      extractedText: '"Force Majeure shall include acts of God, war, blockades, but specifically excludes any delays or failures in performance arising from regulatory inaction, licensing delays, or governmental permits."',
      standardText: '"Force Majeure includes war, strike, acts of God, as well as any delays caused by government action, regulatory inaction, or delay in issuing required permits..."',
      impactAnalysis: "Operator bears the commercial risk of delays caused by licensing authority inaction. Crucial exposure in phase extension applications.",
      portfolioDistribution: [
    ...
    },
    {
      ...
      severity: "Medium",
      status: "Pending",
      extractedText: '"The Operator shall not assign or transfer any share of interest without prior approval. Furthermore, the Government holds the first right of pre-emption on any farm-in transaction proposed by third parties."',
      standardText: '"The Contractor may assign interest with prior consent of the Government, which shall not be unreasonably withheld, without any pre-emption rights held by the Government."',
      impactAnalysis: "Limits farm-down options and increases commercial transaction complexity. Prior notification is required before farm-in discussions.",
      portfolioDistribution: [
    ...
    },
    {
      ...
      severity: "Low",
      status: "Resolved",
      extractedText: '"Either party may terminate this agreement upon default of obligations by providing sixty (60) days written notice of termination to the other party."',
      standardText: '"The Government may terminate this contract upon default by providing ninety (90) days written notice, allowing the Contractor to cure defaults during this period."',
      impactAnalysis: "Reduces the operator's cure-period window by 30 days. Action logged; no immediate action required.",
      portfolioDistribution: [
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="rounded-xl border border-[#D1D9E3] bg-white p-4 shadow-sm flex flex-col justify-between">
          <span className="text-[11px] font-semibold text-[#8892A0] uppercase tracking-wider font-mono">Clauses Analysed</span>
          <div className="text-xl font-bold font-mono text-[#0D1B2A] mt-1">142</div>
        </div>
        <div className="rounded-xl border border-[#D1D9E3] bg-white p-4 shadow-sm flex flex-col justify-between">
          <span className="text-[11px] font-semibold text-[#8892A0] uppercase tracking-wider font-mono">Anomalies Open</span>
          <div className="text-xl font-bold font-mono text-[#C0392B] mt-1">4 <span className="text-[10px] text-[#8892A0] font-normal">/ 5 total</span></div>
        </div>
        <div className="rounded-xl border border-[#D1D9E3] bg-white p-4 shadow-sm flex flex-col justify-between">
          <span className="text-[11px] font-semibold text-[#8892A0] uppercase tracking-wider font-mono">Conflicts Detected</span>
          <div className="text-xl font-bold font-mono text-[#0F7B6C] mt-1">0</div>
        </div>
        <div className="rounded-xl border border-[#D1D9E3] bg-white p-4 shadow-sm flex flex-col justify-between">
          <span className="text-[11px] font-semibold text-[#8892A0] uppercase tracking-wider font-mono">Legal Actions Pending</span>
                  ...
                  {anomalies.map((anom) => (
                    ...
                    >
                      ...
                      <td className="p-3">
                        ...
                        <div className="text-[10px] text-[#8892A0] font-mono mt-0.5">{anom.clauseNum} ({anom.id})</div>
                      </td>
                      ...
                      <td className="p-3 pr-4 text-right">
                        <span className={`inline-block px-1.5 py-0.5 rounded text-[9.5px] font-bold uppercase ${
                          anom.status === "Resolved" ? "bg-[#E3F4F2] text-[#0F7B6C]" :
                          anom.status === "In Review" ? "bg-blue-50 text-[#1A6B8A]" : "bg-[#FDF3E3] text-[#C8922A]"
                        }`}>
                          {anom.status}
                        </span>
                      </td>
          ...
          <div className="lg:col-span-5">
            <div className="rounded-2xl border border-[#D1D9E3] bg-white shadow-sm overflow-hidden flex flex-col h-full">
              ...
              <div className={`p-4 border-b border-[#D1D9E3] text-white flex items-center justify-between ${
                activeAnomaly.severity === "High" ? "bg-[#C0392B]" : activeAnomaly.severity === "Medium" ? "bg-[#C8922A]" : "bg-[#1A6B8A]"
              }`}>
                <div className="space-y-0.5">
                  <span className="text-[9px] uppercase tracking-widest text-white/80 font-mono font-bold">{activeAnomaly.id}</span>
                  <h4 className="text-[12.5px] font-bold truncate max-w-[190px]">{activeAnomaly.category}</h4>
                </div>
              ...
              <div className="p-4 space-y-4 text-xs overflow-y-auto max-h-[385px] flex-1">
                <div className="space-y-1">
                  <span className="text-[9.5px] text-[#8892A0] uppercase font-bold font-mono">Deviation Summary</span>
                ...
                <div className="grid grid-cols-1 gap-2.5 bg-[#F5F6FA] p-3 rounded-xl border">
                  <div className="space-y-0.5">
                    <span className="text-[9px] text-[#C0392B] font-bold uppercase font-mono">Extracted Contract Text ({activeAnomaly.clauseNum})</span>
                  ...
                  <div className="space-y-0.5 border-t pt-2 border-dashed">
                    <span className="text-[9px] text-[#0F7B6C] font-bold uppercase font-mono">Standard Model Template</span>
                    <p className="text-[#0D1B2A] leading-relaxed italic text-[11px]">
                ...
                <div className="space-y-1">
                  <span className="text-[9.5px] text-[#8892A0] uppercase font-bold font-mono">AI Deviation Impact Analysis</span>
                ...
                <div className="space-y-2">
                  <span className="text-[9.5px] text-[#8892A0] uppercase font-bold font-mono">Portfolio Context Distribution</span>
                  <div className="space-y-1.5">
                    {activeAnomaly.portfolioDistribution.map((item, idx) => (
                      <div key={idx} className="flex items-center justify-between p-2 rounded-lg border">
                        ...
                        <span className={`font-mono text-[10.5px] px-2 py-0.5 rounded-md font-semibold ${
                          item.standard ? "text-[#0F7B6C] bg-[#E3F4F2]/50" : "text-[#C0392B] bg-[#FDECEA]"
                        }`}>
                          {item.value}
                        </span>
                      </div>
                ...
                <div className="grid grid-cols-2 gap-3 pt-2">
                  <div className="space-y-1.5">
                    <label className="text-[9.5px] text-[#8892A0] uppercase font-bold font-mono">Assign Legal Owner</label>
                    <select 
                      value={activeAnomaly.owner}
                      onChange={(e) => {
                        const newOwner = e.target.value;
                        setAnomalies(prev => prev.map(a => a.id === activeAnomaly.id ? { ...a, owner: newOwner, status: "In Review" } : a));
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[9.5px] text-[#8892A0] uppercase font-bold font-mono">Due Date</label>
              ...
              <div className="p-3 border-t border-[#D1D9E3] bg-[#F5F6FA]/50 flex items-center justify-end gap-2 shrink-0">
                <Button 
                  ...
                  onClick={() => toast.success("Added to structured corporate legal risk register.")}
                >
                  Add to Legal Queue
                </Button>
                <Button 
        ...
        <div className="rounded-2xl border border-[#D1D9E3] bg-white shadow-sm overflow-hidden flex flex-col space-y-4 p-5 animate-in fade-in duration-200">
          <div className="flex items-center justify-between border-b pb-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-[14px] font-bold text-[#0D1B2A]">Portfolio Clause Comparison Matrix</h3>
                <p className="text-[11px] text-[#8892A0]">Spreadsheet mapping categories as rows and active leases as columns.</p>
              </div>
            </div>
            ...
            <table className="w-full text-left border-collapse text-xs font-mono">
              ...
              <tbody className="divide-y">
                <tr className="hover:bg-slate-50/50">
                  <td className="p-3 pl-4 border-r font-sans font-bold text-[#0D1B2A]">Cost Recovery Cap</td>
                  <td className="p-3 border-r">100% recovery</td>
                  <td className="p-3 border-r bg-red-100 text-[#C0392B] font-extrabold">90% (Anomalous)</td>
                  <td className="p-3 border-r">100% (Standard)</td>
                  <td className="p-3 pr-4">100% (Standard)</td>
                </tr>
                <tr className="hover:bg-slate-50/50">
                  <td className="p-3 pl-4 border-r font-sans font-bold text-[#0D1B2A]">Dispute Seat</td>
                  <td className="p-3 border-r">New Delhi (India)</td>
                  <td className="p-3 border-r bg-red-100 text-[#C0392B] font-extrabold">Singapore (Anomalous)</td>
                  <td className="p-3 border-r">New Delhi (Standard)</td>
                  <td className="p-3 pr-4">New Delhi (Standard)</td>
                </tr>
                <tr className="hover:bg-slate-50/50">
                  <td className="p-3 pl-4 border-r font-sans font-bold text-[#0D1B2A]">Force Majeure</td>
                  <td className="p-3 border-r">Includes Gov Action</td>
                  <td className="p-3 border-r">Standard FM</td>
                  <td className="p-3 border-r bg-amber-100 text-[#C8922A] font-extrabold">Excludes Gov (Anomalous)</td>
                  <td className="p-3 pr-4">Standard FM</td>
                </tr>
                <tr className="hover:bg-slate-50/50">
                  <td className="p-3 pl-4 border-r font-sans font-bold text-[#0D1B2A]">Assignment Pre-emption</td>
                  <td className="p-3 border-r">No pre-emption right</td>
                  <td className="p-3 border-r">No pre-emption</td>
                  <td className="p-3 border-r">No pre-emption</td>
                  <td className="p-3 pr-4 bg-amber-100 text-[#C8922A] font-extrabold">Gov Pre-emption (Anomalous)</td>
                </tr>
                <tr className="hover:bg-slate-50/50">
                  <td className="p-3 pl-4 border-r font-sans font-bold text-[#0D1B2A]">Termination Notice</td>
                  <td className="p-3 border-r">90 days</td>
                  <td className="p-3 border-r text-slate-400">90 days</td>
                  <td className="p-3 border-r bg-blue-50 text-[#1A6B8A] font-bold">60 days (Anomalous)</td>
