  ...
  const [sections, setSections] = useState<ReportSection[]>([
    {
      ...
      content:
        "Well B-14 was drilled to a total depth of 3,847m MD (3,312m TVD) in 14 days, achieving a 96% on-bottom efficiency. A total of 3 NPT events were recorded, accounting for 8.4 hours of lost time, primarily due to stuck pipe at 2,940m in the Cambay Shale section. Final well cost was Rs. 8.47 crore, 6.2% under the approved AFE of Rs. 9.03 crore.",
      status: "complete",
    },
    {
    ...
    },
    {
      ...
      content:
        "[FLAGGED FOR REVIEW] Stuck pipe event at Cambay Shale: Mud density was kept at 1.45 SG which matched shear thresholds, but continuous swabbing during connections indicates geomechanical stress. Mud logs must be verified against geomechanical core summaries to determine safe weight ranges.",
      status: "flagged",
        ...
        <div className="rounded-2xl border border-[#D1D9E3] bg-white p-5 shadow-sm space-y-4">
          <div className="flex items-center justify-between border-b pb-2">
            ...
            <span className="text-[10.5px] font-mono text-[#8892A0]">
              {sourceDocs.filter((d) => d.active).length} active
            </span>
          </div>
            ...
            {sourceDocs.map((doc, idx) => (
              ...
              >
                <div className="space-y-0.5 max-w-[80%]">
                  <p className="text-xs font-semibold text-[#0D1B2A] truncate" title={doc.name}>
                    {doc.name}
                  </p>
        ...
        <div className="rounded-2xl border border-[#D1D9E3] bg-white shadow-sm flex flex-col h-full overflow-hidden">
          <div className="p-4 border-b border-[#D1D9E3] flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-white">
            <div>
              ...
              <p className="text-[11.5px] font-bold text-[#0D1B2A] mt-0.5">
                {reportType} — {subject}
              </p>
            </div>
