...
export function OGSubModule12() {
  ...
  const [gaps, setGaps] = useState<GapItem[]>([
    {
      ...
      description:
        "Methane intensity reporting is now mandatory from Q3 2025. Affected field: Section 6, Emission Data — field currently blank.",
      action: "Add methane intensity calculation before resubmission",
      owner: "Priya Menon",
      filingId: "FILING-2025-1042",
      clauseText:
        'PNGRB Circular 2024-08, Para 3.1(c): "All operators shall include aggregate methane intensity (tCH4 per ktoe of production) as a mandatory disclosure in the Monthly Production Report from the quarter ended 30 September 2025 onward."',
      evidence: [
        "Methane intensity calculation worksheet",
        "Supporting operational data (flaring logs, leak surveys)",
      ],
      resolved: false,
    },
    {
      ...
      description:
        "Emergency response contact template format updated Q3 2025. Affected section: Section 9B — current format does not match updated template.",
      action: "Update contact entries to new format at next filing",
      owner: "Rajesh Kumar",
      filingId: "FILING-2025-1042",
      clauseText:
        'OISD-GS-RP-04, Section 9.2: "Standard emergency response sheets must specify regional mobile phone grids in 10-digit primary format (+91 prefix) and verify control room status annually."',
    ...
    },
    {
      ...
      description:
        "Well integrity certification period reduced from 24 to 18 months per DGH circular. Affected field: Section 4D — two well certs shown as valid at 24-month interval.",
      action: "Review well certification schedule against new 18-month requirement",
      owner: "Sushant Verma",
      filingId: "FILING-2025-1038",
      clauseText:
        'DGH Guidelines 2024, Ch. 2.1: "Well integrity diagnostic charts (WIC) must be updated and signed by a certified geomechanist every 18 months for onshore active producers."',
    ...
    },
    {
      ...
      filingId: "FILING-2025-1042",
      clauseText:
        'MoPNG circular 2025/11: "Upstream operators are encouraged to disclose water recycle ratios and carbon offset credits voluntarily on a quarterly cycle."',
      evidence: [],
      resolved: false,
    },
  ]);
  ...
  const handleResolveGap = (gapId: string) => {
    ...
    if (activeGap.severity === "Critical") {
      ...
      if (!allChecked) {
        toast.error(
          "Please complete all evidence checklist items before resolving a Critical gap.",
        );
        return;
      }
    }
  ...
  const handleRunScan = () => {
    ...
    setScanProgress(0);
    toast.loading(
      "Running comprehensive compliance check against latest DGH, PNGRB, and OISD knowledge bases...",
    );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="rounded-xl border border-[#D1D9E3] bg-white p-4 shadow-sm flex flex-col justify-between">
          <span className="text-[11px] font-semibold text-[#8892A0] uppercase tracking-wider">
            Filings Scanned
          </span>
                ...
                {filteredGaps.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="p-12 text-center text-[#8892A0]">
                      ...
                      <p className="font-semibold text-[#0D1B2A] text-[13px]">
                        No compliance gaps found
                      </p>
                      <p className="text-[11px] mt-0.5">
                        All files adhere to active regulatory models.
                      </p>
                    </td>
                  </tr>
                ) : (
                  filteredGaps.map((gap) => (
                    ...
                    >
                      ...
                      <td className="p-3">
                        ...
                        <div className="text-[10px] text-[#8892A0] mt-0.5 font-mono">
                          {gap.source}
                        </div>
                      </td>
            ...
            <div className="p-5 space-y-4 text-xs overflow-y-auto max-h-[385px] flex-1">
              <div className="space-y-1">
                ...
                <p className="font-semibold text-[#0D1B2A]">
                  {activeGap.source} ({activeGap.filingId})
                </p>
              </div>
              ...
              <div className="bg-[#F5F6FA] rounded-xl p-3 border border-[#D1D9E3] space-y-1">
                <span className="text-[9.5px] font-bold text-[#1A6B8A] uppercase tracking-wider font-mono">
                  Regulatory Clause Text
                </span>
                ...
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setGaps((prev) =>
                        prev.map((g) => (g.id === activeGap.id ? { ...g, owner: newOwner } : g)),
                      );
                    ...
                    {activeGap.evidence.map((item, idx) => {
                      ...
                      return (
                        <div
                          ...
                          onClick={() =>
                            setEvidenceChecked((prev) => ({ ...prev, [key]: !isChecked }))
                          }
                        >
                          <div
                            className={`h-4.5 w-4.5 border rounded-sm flex items-center justify-center shrink-0 transition-all ${
                              isChecked
                                ? "bg-[#0F7B6C] border-[#0F7B6C] text-white"
                                : "border-[#D1D9E3] bg-white"
                            }`}
                          >
