...
import {
  FileText, ShieldCheck, ShieldAlert, CheckCircle2, XCircle, AlertTriangle,
  Clock, ArrowRight, UserPlus, RefreshCw, UserCheck, Shield
} from "lucide-react";
...
export function OGSubModule63() {
  ...
  const [logs, setLogs] = useState<LogEntry[]>([
    { time: "11:30:12 IST", action: "PTW Check: Vijay Singh ✓, Ramesh Nair ✗ (Gas Testing L2 expired), Deepak Arora ✓", status: "blocked" }
  ]);
  ...
  const handleApplySubstitute = () => {
    ...
    setLogs(prev => [
      { time: "11:32:05 IST", action: "Substituted Ramesh Nair with Suresh Kumar. Running verification...", status: "resolved" },
      { time: "11:32:07 IST", action: "Suresh Kumar check: Gas Testing L2 ✓ (exp. 14 Sep 2026). All workers now qualified.", status: "success" },
      ...prev
    ]);
  };
  ...
  const handleSubmitPermit = () => {
    ...
    setLogs(prev => [
      { time: "11:34:10 IST", action: "Permit PTW-2025-1163 officially issued by Priya Menon (HSE Manager).", status: "success" },
      ...prev
    ]);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-[#D1D9E3] bg-white p-4 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-[#8892A0] font-semibold">Location: Storage Tank T-04 | Issuer: Priya Menon (HSE Manager)</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {permitIssued ? (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 rounded-2xl border border-[#D1D9E3] bg-white shadow-sm overflow-hidden flex flex-col">
          <div className="p-4 border-b border-[#D1D9E3] bg-slate-50/50 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-xs font-bold text-[#1A6B8A] uppercase tracking-wider">Crew Qualification Verification</h3>
            </div>
            ...
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-[#D1D9E3]">
                ...
                <tr className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="py-3.5 px-2 text-center">
                    <span className="inline-flex items-center gap-1 text-amber-600 font-semibold bg-amber-50 px-2 py-0.5 rounded-full border border-amber-200 text-[10px]" title="Advisory Warning - Expires soon">
                      ⚠ Expiring <span className="font-mono text-[9px] text-[#8892A0]">(11 Nov)</span>
                    </span>
                  </td>
                </tr>
                ...
                {!isSubstituted ? (
                  <tr className="bg-red-50/30 hover:bg-red-50/50 transition-colors border-l-4 border-l-[#C0392B]">
                    ...
                    <td className="py-3.5 px-2 text-center">
                      <span className="inline-flex items-center gap-1 text-[#0F7B6C] font-semibold bg-[#E3F4F2] px-2 py-0.5 rounded-full border border-[#0F7B6C]/10 text-[10px]">
                        ✓ Valid <span className="font-mono text-[9px] text-[#8892A0]">(20 Aug)</span>
                      </span>
                    </td>
                    <td className="py-3.5 px-2 text-center">
                      <span className="inline-flex items-center gap-1 text-[#C0392B] font-bold bg-[#FDECEA] px-2 py-0.5 rounded-full border border-[#C0392B]/10 text-[10px]">
                        ✗ Expired <span className="font-mono text-[9px] text-[#8892A0]">(29 Oct)</span>
                      </span>
                    </td>
                    <td className="py-3.5 px-2 text-center">
                      <span className="inline-flex items-center gap-1 text-[#0F7B6C] font-semibold bg-[#E3F4F2] px-2 py-0.5 rounded-full border border-[#0F7B6C]/10 text-[10px]">
                        ✓ Valid <span className="font-mono text-[9px] text-[#8892A0]">(11 Nov)</span>
                      </span>
                    </td>
                  </tr>
                ) : (
                  <tr className="bg-blue-50/30 hover:bg-blue-50/50 transition-colors border-l-4 border-l-[#1A6B8A]">
                    <td className="py-3.5 px-4">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge className="bg-blue-50 text-[#1A6B8A] border-blue-200 border text-[8px] font-bold">SUBSTITUTE</Badge>
                      </div>
                      <p className="text-[10px] text-[#8892A0]">Gas Tester (Substituted)</p>
                    </td>
                    <td className="py-3.5 px-2 text-center">
                      <span className="inline-flex items-center gap-1 text-[#0F7B6C] font-semibold bg-[#E3F4F2] px-2 py-0.5 rounded-full border border-[#0F7B6C]/10 text-[10px]">
                        ✓ Valid <span className="font-mono text-[9px] text-[#8892A0]">(15 Feb)</span>
                      </span>
                    </td>
                    <td className="py-3.5 px-2 text-center">
                      <span className="inline-flex items-center gap-1 text-[#0F7B6C] font-semibold bg-[#E3F4F2] px-2 py-0.5 rounded-full border border-[#0F7B6C]/10 text-[10px]">
                        ✓ Valid <span className="font-mono text-[9px] text-[#8892A0]">(14 Sep)</span>
                      </span>
                    </td>
                    <td className="py-3.5 px-2 text-center">
                      <span className="inline-flex items-center gap-1 text-[#0F7B6C] font-semibold bg-[#E3F4F2] px-2 py-0.5 rounded-full border border-[#0F7B6C]/10 text-[10px]">
        ...
        <div className="rounded-2xl border border-[#D1D9E3] bg-white shadow-sm overflow-hidden flex flex-col p-5 space-y-4">
          <h3 className="text-xs font-bold text-[#1A6B8A] uppercase tracking-wider border-b pb-1.5">PTW Submission Gate</h3>
          ...
          {permitIssued ? (
            <div className="bg-[#E3F4F2] border border-[#0F7B6C]/20 rounded-xl p-4 text-center space-y-3 flex-1 flex flex-col justify-center">
              ...
              <div>
                ...
                <p className="text-[11px] text-[#0F7B6C]/80 mt-1">PTW-2025-1163 is now active. Crew cleared to proceed with safety compliance verified.</p>
              </div>
              <Button size="sm" onClick={handleReset} variant="outline" className="h-8 border-[#0F7B6C]/30 text-[#0F7B6C] hover:bg-teal-50 text-[11px] font-bold mt-2">
            ...
            <div className="flex-1 flex flex-col justify-between space-y-4">
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-bold text-[#8892A0] uppercase">Assign Substitute Worker</label>
                ...
                {subName.toLowerCase() === "suresh kumar" ? (
                  <p className="text-[10px] text-[#0F7B6C] font-semibold">✓ Suresh Kumar is qualified (Gas Testing L2 current)</p>
                ) : (
                  <p className="text-[10px] text-[#8892A0] italic">Type "Suresh Kumar" to pre-qualify a substitute.</p>
                )}
              </div>
              ...
              <div className="relative">
                ...
                <div className="absolute inset-0 bg-white/80 backdrop-blur-[0.5px] rounded-xl flex items-center justify-center z-10 p-2 text-center">
                  <div className="space-y-1">
                    ...
                    <p className="text-[10.5px] font-bold text-[#C0392B]">Compliance Block Active</p>
                  </div>
                </div>
            ...
            <div className="flex-1 flex flex-col justify-between space-y-4">
              <div className="bg-[#E3F4F2] border border-[#0F7B6C]/20 rounded-xl p-4 space-y-3">
                ...
                <p className="text-[11.5px] text-[#0D1B2A] leading-relaxed">
                  Suresh Kumar substituted successfully for Ramesh Nair. All crew members pass verification.
                </p>
              </div>

              <div className="flex items-center justify-between text-[11.5px]">
                <button onClick={handleReset} className="text-[#8892A0] hover:text-[#0D1B2A] text-xs font-semibold flex items-center gap-1">
      ...
      <div className="rounded-2xl border border-[#D1D9E3] bg-white p-5 shadow-sm space-y-3">
        <h3 className="text-xs font-bold text-[#1A6B8A] uppercase tracking-wider">Permit Verification Audit Logs</h3>
        <div className="space-y-2 max-h-[140px] overflow-y-auto font-mono text-[10.5px]">
          {logs.map((log, idx) => {
            ...
            return (
              <div key={idx} className="flex items-start gap-3 p-2 rounded-lg border bg-slate-50/50">
