...
export function OGSubModule31() {
  ...
  const [jsaSteps, setJsaSteps] = useState<JsaStep[]>([
    {
      ...
      severity: 4,
      controls:
        "Full face shield + chemical-resistant gloves + apron mandatory. Cordon off 10m radius. Wind sock checked — upwind positioning only.",
    ...
    },
    {
      ...
      severity: 3,
      controls:
        "Mixing only in ventilated area. SCBA on standby. Eye wash <10m. Temperature monitoring — abort if >45°C during mixing.",
    ...
    },
    {
      ...
      severity: 3,
      controls:
        "Pressure test to 1.5× MAWP before acid injection. All personnel behind blast shield during pressure test. PTW Hot Work endorsement required for all open-flame equipment nearby.",
      ...
      setGenStep((prev) => {
        if (prev >= genStepsTexts.length - 1) {
          ...
          setTimeout(() => {
            ...
            setEditLog([
              {
                field: "Regenerated Draft",
                time: new Date().toLocaleTimeString(),
                user: "Fortiv AI Engine",
              },
            ]);
  ...
  const getRiskLevel = (score: number) => {
    if (score >= 12)
      return {
        label: "CRITICAL",
        color: "bg-[#C0392B] text-white animate-pulse",
        border: "border-[#C0392B]",
      };
    if (score >= 8)
      return { label: "HIGH", color: "bg-[#C8922A] text-white", border: "border-[#C8922A]" };
    if (score >= 4)
      return { label: "MEDIUM", color: "bg-[#1A6B8A] text-white", border: "border-[#1A6B8A]" };
  ...
  const saveEdit = (index: number) => {
    ...
    setEditLog((prev) => [
      {
        field: `Edited Step ${index + 1}`,
        time: new Date().toLocaleTimeString(),
        user: "Rajesh Kumar (Site Supervisor)",
      },
      ...prev,
    ]);
  ...
  const removeStep = (idx: number) => {
    const filtered = jsaSteps
      .filter((_, i) => i !== idx)
      .map((step, i) => ({ ...step, stepNo: i + 1 }));
    setJsaSteps(filtered);
    setEditLog((prev) => [
      {
        field: `Deleted Step ${idx + 1}`,
        time: new Date().toLocaleTimeString(),
        user: "Rajesh Kumar (Site Supervisor)",
      },
      ...prev,
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div className="lg:col-span-5 space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              >
                ...
                <option value="Well Pad C - Drilling Platform">
                  Well Pad C - Drilling Platform
                </option>
        ...
        <div className="rounded-2xl border border-[#D1D9E3] bg-white p-5 shadow-sm space-y-3">
          <h4 className="text-[12px] font-bold text-[#0D1B2A] uppercase tracking-wider text-[#1A6B8A]">
            JSA Modification Audit Trail
          </h4>
          <div className="space-y-2">
            {editLog.map((log, idx) => (
              <div
                key={idx}
                className="flex items-start justify-between text-[11px] border-b pb-2 last:border-none last:pb-0"
              >
      ...
      <div className="lg:col-span-7">
        <div className="rounded-2xl border border-[#D1D9E3] bg-white shadow-sm flex flex-col overflow-hidden h-full">
          ...
          <div className="p-5 border-b border-[#D1D9E3] bg-white flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <div className="flex items-center gap-2">
                <Badge className="bg-blue-50 text-[#1A6B8A] border-blue-100 font-mono text-[10.5px]">
                  M3.1 JSA
                </Badge>
              ...
              <p className="text-[11px] text-[#8892A0] mt-1">
                Linked Activity: <span className="font-semibold text-[#0D1B2A]">{workType}</span> at{" "}
                <span className="font-semibold text-[#0D1B2A]">{location}</span>
              </p>
            </div>

            {isGenerated && (
              <Badge
                className={`${globalRisk.color} font-bold text-[11px] px-2.5 py-0.5 rounded-full border`}
              >
            ...
            {isGenerating ? (
              <div className="py-24 text-center space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] text-[#8892A0]">
                    Fortiv Safety AI Engine processing...
                  </p>
                </div>
              </div>
            ) : isGenerated ? (
              <div className="space-y-4">
                {jsaSteps.map((step, idx) => (
                  <div
                    key={idx}
                    className="rounded-xl border border-[#D1D9E3] bg-white p-4 shadow-sm space-y-3 relative group"
                  >
                    {/* Step Title & Actions */}
                      ...
                      {editingStepIndex !== idx ? (
                        <div className="flex items-center gap-1.5 opacity-60 group-hover:opacity-100 transition-opacity">
                          <button
                            ...
                            className="p-1 rounded hover:bg-slate-100 text-[#1A6B8A]"
                            title="Edit Step"
                          >
                          ...
                          <button
                            ...
                            className="p-1 rounded hover:bg-slate-100 text-[#C0392B]"
                            title="Remove Step"
                          >
                    ...
                    {editingStepIndex === idx && editFields ? (
                      <div className="space-y-3 pt-1 text-xs">
                        <div className="grid grid-cols-2 gap-2">
                          <div className="space-y-1">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setEditFields({ ...editFields, activity: e.target.value })
                              }
                              className="w-full h-8 border rounded px-2"
                            />
                          </div>
                          <div className="space-y-1">
                            <label className="text-[10px] font-bold text-[#8892A0]">
                              Reg Reference
                            </label>
                            <input
                              type="text"
                              ...
                              onChange={(e) =>
                                setEditFields({ ...editFields, reference: e.target.value })
                              }
                              className="w-full h-8 border rounded px-2"
                            />
                          </div>
                        ...
                        <div className="space-y-1">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setEditFields({ ...editFields, hazard: e.target.value })
                            }
                            className="w-full h-8 border rounded px-2"
                          />
                        </div>
                        ...
                        <div className="space-y-1">
                          ...
                          <textarea
                            ...
                            onChange={(e) =>
                              setEditFields({ ...editFields, controls: e.target.value })
                            }
                            rows={2}
                        ...
                        <div className="space-y-1">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setEditFields({ ...editFields, emergency: e.target.value })
                            }
                            className="w-full h-8 border rounded px-2"
                          />
                        </div>
                        ...
                        <div className="flex justify-end gap-2 pt-1.5 border-t">
                          ...
                          <Button
                            ...
                            size="sm"
                            className="h-7 bg-[#0F7B6C] text-white text-[11px]"
                          >
                            Save Step
                          </Button>
                        </div>
                      ...
                      <div className="space-y-2 text-xs">
                          ...
                          <div className="text-right shrink-0">
                            ...
                            <div className="flex items-center gap-1.5 mt-0.5 justify-end">
                              <span className="font-mono font-bold text-[11.5px]">
                                {step.likelihood} × {step.severity} =
                              </span>
                              <span
                                className={`font-mono font-bold px-1.5 py-0.5 rounded text-[11px] ${getRiskLevel(getRiskScore(step.likelihood, step.severity)).color}`}
                              >
                        ...
                        <div className="space-y-1 bg-[#F5F6FA] p-2.5 rounded-lg border border-[#D1D9E3]/50">
                          <span className="text-[10px] font-bold text-[#1A6B8A] uppercase tracking-wider block">
                            Required Controls
                          </span>
                ...
                <Button
                  onClick={addStep}
                  variant="outline"
                  className="w-full border-dashed border-[#D1D9E3] hover:border-[#1A6B8A] text-xs h-9 flex items-center justify-center gap-1.5"
                >
              ...
              <div className="py-24 text-center space-y-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-[#8892A0]">
                    Configure work parameters and click "Generate Safety JSA".
                  </p>
                </div>
              </div>
            )}
