...
export function OGSubModule33() {
  ...
  const [analysisFramework, setAnalysisFramework] = useState<"5-Why" | "Bowtie" | "Fault Tree">(
    "5-Why",
  );

  // AI analysis output state
  const [immediateCause, setImmediateCause] = useState(
    "Wet surface on elevated platform access ladder",
  );
  const [whys, setWhys] = useState([
    { label: "Why 1 (Direct Source)", text: "No anti-slip matting or grating on platform surface" },
    {
      label: "Why 2 (Equipment Setup)",
      text: "Platform specification did not include anti-slip requirement",
    },
    {
      label: "Why 3 (Planning Gap)",
      text: "Existing JSA for elevated work did not flag wet-surface risk",
    },
    {
      label: "Why 4 (Process Detail)",
      text: "JSA template for height work lacks weather condition check",
    },
    {
      label: "Why 5 (Systemic Root)",
      text: "Pre-task inspection checklist does not include surface hazard assessment for elevated work in wet conditions",
    },
  ]);
  ...
  const [correctiveActions, setCorrectiveActions] = useState<CorrectiveAction[]>([
    {
      id: 1,
      description: "Install non-slip grating on all elevated platforms",
      priority: "HIGH",
      owner: "Sushant Verma",
      due: "04 Nov 2025",
      status: "Open",
    ...
    {
      id: 2,
      description: "Add 'wet surface hazard check' to JSA template for outdoor elevated work",
      priority: "HIGH",
      owner: "Priya Menon",
      due: "31 Oct 2025",
      status: "Open",
    ...
    {
      id: 3,
      description: "Install tool bag holder/hoist at all elevated platform access points",
      priority: "MED",
      owner: "Ramesh Nair",
      due: "14 Nov 2025",
      status: "Open",
  ...
  const requiresRegulatoryNotification =
    severity === "Serious" ||
    severity === "Fatal" ||
    incidentType === "LTI" ||
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div className="lg:col-span-6 rounded-2xl border border-[#D1D9E3] bg-white p-5 shadow-sm space-y-4">
        <div className="flex items-center justify-between border-b pb-3">
          ...
          <Badge className="bg-slate-100 text-slate-700 hover:bg-slate-100 font-mono text-[10.5px]">
            INC-2025-0089
          </Badge>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full h-9 rounded-lg border border-[#D1D9E3] bg-[#F5F6FA] px-2 text-xs font-mono"
              readOnly
            />
          </div>
          <div className="space-y-1.5">
          ...
          <div className="space-y-1.5">
            ...
            <input
              type="text"
              defaultValue="Well No. 12"
              className="w-full h-9 rounded-lg border border-[#D1D9E3] bg-[#F5F6FA] px-2 text-xs font-semibold"
              readOnly
            />
          </div>
        </div>
              ...
              {(["5-Why", "Bowtie", "Fault Tree"] as const).map((frame) => (
                <button
                  ...
                  className={`text-[10px] font-bold px-2 py-1 rounded-md transition-all ${
                    analysisFramework === frame
                      ? "bg-white text-[#0D1B2A] shadow-xs"
                      : "text-[#8892A0] hover:text-[#0D1B2A]"
                  }`}
                >
                  {frame}
                </button>
          ...
          {isAnalyzing ? (
            ...
            <div className="space-y-4 text-xs">
                  ...
                  <div className="space-y-2 pl-3 border-l-2 border-slate-100">
                    {whys.map((why, idx) => (
                      <div key={idx} className="space-y-0.5">
                        <p className="text-[9.5px] font-bold text-[#1A6B8A] uppercase">
                          {why.label}
                        </p>
              ...
              <div className="space-y-2 pt-3 border-t">
                <span className="text-[10.5px] font-bold text-[#8892A0] uppercase tracking-wider block">
              ...
              <div className="space-y-2.5 pt-3 border-t">
                ...
                <div className="space-y-2">
                  {correctiveActions.map((action) => (
                    <div
                      key={action.id}
                      className="p-2.5 rounded-lg border bg-slate-50 flex items-start justify-between gap-3"
                    >
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-2 text-[10px] text-[#8892A0]">
                          <span>
                            Owner: <strong className="text-[#1A6B8A]">{action.owner}</strong>
                          </span>
                          <span>•</span>
                          <span>
                            Due: <strong className="font-mono">{action.due}</strong>
                          </span>
                        </div>
                      </div>
