...
import { 
  ...
  HelpCircle, ExternalLink, Download, Send, Check
} from "lucide-react";
...
export function OGSubModule11() {
  ...
  const [documents, setDocuments] = useState<DocumentItem[]>([
    { name: "Oct_2025_Production_Summary_XYZ07.pdf", size: "3.2 MB", pages: 12, status: "Ready", type: "PDF" },
    { name: "Oct_2025_DDR_Consolidated_XYZ07.xlsx", size: "1.8 MB", pages: 0, status: "Ready", type: "Excel" },
    { name: "Oct_2025_Well_Test_Report_W3.pdf", size: "840 KB", pages: 4, status: "Processing", type: "PDF" }
  ]);

  // Form Fields state
  const [formFields, setFormFields] = useState<FormField[]>([
    { id: "Field 1A", label: "Operator Name", value: "Fortiv Energy India Ltd.", source: "Page 1, Header Block, Confidence: 100%", confidence: 100, status: "success" },
    { id: "Field 1B", label: "Reporting Block ID", value: "Block XYZ-07", source: "Page 1, Header Block, Confidence: 100%", confidence: 100, status: "success" },
    { id: "Field 3A", label: "Crude Oil Production (bbl)", value: "48,340", source: "Page 2, Para 3, Confidence: 99%", confidence: 99, status: "success" },
    { id: "Field 7B", label: "Gas Flared (MMSCFD)", value: "2.1", source: "Page 4, Table 1, Confidence: 98%", confidence: 98, status: "success" },
    { id: "Field 9C", label: "Active Producer Wells Count", value: "14", source: "Page 1, Summary Section, Confidence: 100%", confidence: 100, status: "success" },
    { id: "Field 10D", label: "Shut-in Wells Count", value: "2", source: "Page 1, Summary Section, Confidence: 100%", confidence: 100, status: "success" },
    { id: "Field 11E", label: "Water Injected Volume (bbl)", value: "18,200", source: "Page 3, Section 4, Confidence: 97%", confidence: 97, status: "success" },
    { id: "Field 12A", label: "Avg Water Injection Pressure (psi)", value: "2,400", source: "Page 3, Section 4, Confidence: 95%", confidence: 95, status: "success" },
    { id: "Field 12F", label: "Water Disposal Volume (bbl)", value: "", source: "", confidence: 0, status: "danger", flagged: true, errorMsg: "Water disposal volume missing from source document" }
  ]);
      ...
      <div className="lg:col-span-5 space-y-6 flex flex-col">
        ...
        <div className="rounded-2xl border border-[#D1D9E3] bg-white p-5 shadow-sm space-y-4">
          <div className="flex flex-col gap-1">
            <h3 className="text-[15px] font-semibold text-[#0D1B2A]">Form & Period Configuration</h3>
            <p className="text-[11.5px] text-[#8892A0]">Select the target regulatory report to auto-populate.</p>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <label className="text-[11px] font-semibold text-[#1A6B8A] uppercase tracking-wider">Regulatory Form</label>
              <select 
                value={selectedForm} 
            ...
            <div className="space-y-1.5">
              <label className="text-[11px] font-semibold text-[#1A6B8A] uppercase tracking-wider">Filing Period</label>
              <select 
        ...
        <div className="rounded-2xl border border-[#D1D9E3] bg-white p-5 shadow-sm flex-1 flex flex-col space-y-4">
          ...
          <div className="border border-dashed border-[#D1D9E3] bg-[#F5F6FA] hover:bg-slate-50 transition-colors rounded-xl p-6 text-center cursor-pointer relative">
            ...
            <p className="text-[12px] font-semibold text-[#0D1B2A]">Drag & drop operational reports here</p>
        ...
        <div className="rounded-2xl border border-[#D1D9E3] bg-white shadow-sm flex flex-col h-full overflow-hidden">
          ...
          <div className="p-5 border-b border-[#D1D9E3] bg-white space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2.5">
              <div>
                <h3 className="text-[15px] font-bold text-[#0D1B2A] tracking-tight">{selectedForm}</h3>
                <p className="text-[11.5px] text-[#8892A0]">
                  Authority: <span className="font-semibold text-[#1A6B8A]">PNGRB</span> • Period: <span className="font-semibold text-[#1A6B8A]">{selectedPeriod}</span>
                </p>
              </div>
              ...
              <div className="space-y-1">
                <div className="flex items-center justify-between text-[11.5px] font-medium">
                  ...
                  <span className="font-mono text-[#0D1B2A] font-semibold">
                    {countFilled}/{formFields.length} fields populated ({Math.round((countFilled/formFields.length)*100)}%)
                  </span>
                </div>
                <Progress 
            ...
            <TooltipProvider delayDuration={150}>
              <div className="rounded-xl border border-[#D1D9E3] bg-white p-5 shadow-sm space-y-4 font-sans text-xs">
                <div className="text-center border-b pb-4 mb-4 space-y-1">
                  <p className="font-bold uppercase tracking-wider text-[10.5px] text-[#8892A0]">Government of India</p>
                  <p className="font-bold text-[13px] text-[#0D1B2A]">Petroleum and Natural Gas Regulatory Board</p>
                  <p className="text-[10px] text-[#8892A0] italic">Form I: Monthly Field Production and Operational Return</p>
                </div>
                ...
                {!isPopulated && !isPopulating ? (
                  <div className="py-20 text-center space-y-3">
                    ...
                    <div>
                      <p className="text-[13px] font-semibold text-[#0D1B2A]">Form template not populated</p>
                      <p className="text-[11px] text-[#8892A0] mt-1">Upload operational summaries and click "Auto-populate from documents".</p>
                    </div>
                  </div>
                ) : (
                      ...
                      >
                        <div className="space-y-0.5 md:max-w-[45%]">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="font-semibold text-[#0D1B2A] text-[11.5px]">{field.label}</span>
                          </div>
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <div className="h-8 min-w-[120px] max-w-[200px] border border-[#0F7B6C]/20 bg-[#E3F4F2]/50 hover:bg-[#E3F4F2] cursor-help px-3 rounded-lg flex items-center justify-between transition-colors">
                                  <span className="font-mono font-bold text-[#0D1B2A] text-[12px] truncate">{field.value}</span>
                                  <HelpCircle className="h-3 w-3 text-[#0F7B6C] shrink-0 ml-1.5" />
                                </div>
                              </TooltipTrigger>
                              <TooltipContent side="top" className="bg-[#0D1B2A] border-none text-white max-w-[280px] p-3 rounded-lg shadow-lg">
                                <div className="space-y-1 text-[11px] font-sans">
                                  ...
                                  <div className="flex items-center gap-1.5 mt-2 border-t border-slate-700 pt-1.5">
                                    ...
                                    <span className={`text-[10px] font-mono px-1 rounded-sm ${getConfidenceColor(field.confidence)}`}>
            ...
            <div className="flex items-center gap-2.5 w-full sm:w-auto">
              ...
              <Button 
                ...
                onClick={() => {
                  ...
                  setTimeout(() => {
                    toast.dismiss();
                    toast.success("Filing successfully transmitted to PNGRB portal! Portal Transaction ID: TXN-PNGRB-2025-0982.");
                  }, 1500);
                }}
